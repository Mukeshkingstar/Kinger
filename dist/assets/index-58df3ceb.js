 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const RD=5*60,AD=yv("authIdTokenMaxAge")||RD;let Qm=null;const DD=t=>async e=>{const n=e&&await e.getIdTokenResult(),r=n&&(new Date().getTime()-Date.parse(n.issuedAtTime))/1e3;if(r&&r>AD)return;const i=n==null?void 0:n.token;Qm!==i&&(Qm=i,await fetch(t,{method:i?"POST":"DELETE",headers:i?{Authorization:`Bearer ${i}`}:{}}))};function g1(t=Sv()){const e=vd(t,"auth");if(e.isInitialized())return e.getImmediate();const n=UR(t,{popupRedirectResolver:ID,persistence:[OA,SA,l1]}),r=yv("authTokenSyncURL");if(r){const s=DD(r);gA(n,s,()=>s(n.currentUser)),mA(n,o=>s(o))}const i=gv("auth");return i&&lA(n,`http://${i}`),n}ND("Browser");const xD={apiKey:"AIzaSyDWQMRrqtWrnBpCK082PksAp_1jHAj0JRk",authDomain:"kinger-0.firebaseapp.com",projectId:"kinger-0",storageBucket:"kinger-0.appspot.com",messagingSenderId:"460556548889",appId:"1:460556548889:web:3e0a9634ab200eff87e388",measurementId:"G-44S2HCX80X"},PD=Ev(xD),y1=fR(PD),OD=()=>{const[t,e]=N.useState([]),n=Nw(y1,"Post");return N.useEffect(()=>{(async()=>{const i=await DR(n);e(i.docs.map(s=>({...s.data(),id:s.id})))})()}),K($r,{children:K($r,{children:t.map(r=>Et($r,{children:[" ",K("center",{children:Et("div",{className:"show-post",children:[Et("div",{className:"user",children:[K("img",{src:r.author.id,height:50}),K("h4",{children:r.author.name})]}),K("p",{children:r.post})]})})]}))})})},LD=({setIsAuth:t})=>{const e=g1(),n=new Vt,r=fd(),i=()=>{BA(e,n).then(s=>{localStorage.setItem("isAuth",!0);const o=s.user.photoURL;localStorage.setItem("Photo",o),t(!0),r("/")})};return localStorage.getItem("Photo"),K($r,{children:K("center",{children:Et("div",{className:"login-box",children:[K("h2",{children:"Join Toaday"}),K("p",{children:"Sign in one of the provider"}),K("button",{onClick:i,children:"Log in With Google"})]})})})},MD=()=>{const[t,e]=N.useState(""),n=Nw(y1,"Post"),r=g1(),i=fd();return Et($r,{children:[" ",Et("center",{children:[Et("div",{className:"post-write",children:[K("h1",{children:"Create A Post"}),K("input",{type:"post",placeholder:"What is your Mind",onChange:a=>{e(a.target.value)}})," ",K("br",{}),K("button",{onClick:async()=>{await xR(n,{post:t,author:{name:r.currentUser.displayName,id:r.currentUser.uid,id:r.currentUser.photoURL}}),i("/")},children:"Post"})]}),K("div",{className:"logout",children:K("button",{onClick:()=>{yA(r).then(()=>{localStorage.clear(),window.location.pathname="/"})},children:"Log Out"})})]})]})},$D=()=>{const[t,e]=N.useState(!1);return K($r,{children:Et(K_,{children:[Et("div",{className:"nav",children:[Et("div",{className:"logo",children:[K("img",{src:"https://cdn-icons-png.flaticon.com/512/4015/4015248.png",height:50})," ",K("h2",{children:" Kinger"})]}),K("div",{className:"join",children:t?K(Op,{to:"/post",children:K("button",{children:"Post"})}):K(Op,{to:"/login",children:K("button",{children:"Join Now"})})})]}),Et(B_,{children:[K(Go,{path:"/",element:K(OD,{})}),K(Go,{path:"/Post",element:K(MD,{})}),K(Go,{path:"login",element:K(LD,{setIsAuth:e})})]})]})})};ju.createRoot(document.getElementById("root")).render(K(rg.StrictMode,{children:K($D,{})}));
